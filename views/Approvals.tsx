...
export default function Approvals() {
  ...
  const processSingleRequest = async (req: LeaveRequest, approved: boolean) => {
      ...
      // 條件：單據最終核准，且類型為加班或抵休
      // 修改：移除 !req.isCancellationRequest 的限制。
      // 原因：若銷假申請最終核准 (isFinalApproval = true)，狀態變為 CANCELLED，
      // 此時也需要重新結算以「扣除」該筆時數 (更新 Snapshot)。
      if (approved && isFinalApproval && (req.type === LeaveType.OVERTIME || req.type === LeaveType.COMPENSATORY)) {
