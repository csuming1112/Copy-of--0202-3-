  ...
  const executeAction = async () => {
      ...
      try {
        if (type === 'APPLY_CANCEL') {
            ...
            let totalSteps = req.totalSteps || 3; 

            if (userGroup) {
                totalSteps = userGroup.steps.length;
                // 根據職稱規則判斷是否可以跳過某些關卡
                const rule = userGroup.titleRules.find(r => r.jobTitle === user.jobTitle);
                if (rule) {
                    totalSteps = Math.min(rule.maxLevel, totalSteps);
                }
            }
            ...
            // --- 自動觸發結算 (確認基準) ---
            // 當單據被撤回 (變成 Cancelled)，系統需重新計算該月有效單據總和，
            // 此動作會將該筆單據排除在計算之外，從而「扣除」該筆時數。
            if (req.type === LeaveType.OVERTIME || req.type === LeaveType.COMPENSATORY) {
                const reqDate = new Date(req.startDate);
                db.recalculateUserBalanceChain(req.userId, reqDate.getFullYear(), reqDate.getMonth() + 1)
                  .catch(err => console.error("Auto settlement error (Withdraw):", err));
            }
        }
            ...
            // --- 自動觸發結算 ---
            // 當中止銷假，狀態變回 Approved，需加回時數 (確認基準)
            if (req.type === LeaveType.OVERTIME || req.type === LeaveType.COMPENSATORY) {
                const reqDate = new Date(req.startDate);
                db.recalculateUserBalanceChain(req.userId, reqDate.getFullYear(), reqDate.getMonth() + 1)
                  .catch(err => console.error("Auto settlement error (Abort Cancel):", err));
            }
        }
        else if (type === 'DELETE') {
